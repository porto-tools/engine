...
import { reverseVideoDescriptor } from "./reverse-video";

// porto-tools' vitest.config.ts uses the default Node environment (no browser).
// The reverse-video pipeline requires the @ffmpeg/ffmpeg MT runtime, which:
...
describe("reverseVideoDescriptor", () => {
  it("has the correct descriptor fields", () => {
    expect(reverseVideoDescriptor.id).toBe("reverse-video");
    expect(reverseVideoDescriptor.fromLabel).toBe("MP4");
    expect(reverseVideoDescriptor.toLabel).toBe("Reversed MP4");
    expect(reverseVideoDescriptor.newExtension).toBe("mp4");
    expect(reverseVideoDescriptor.accept).toContain("video/mp4");
    expect(typeof reverseVideoDescriptor.loadEngine).toBe("function");
    expect(typeof reverseVideoDescriptor.setupSizeLabel).toBe("string");
    // MT core.
    expect(reverseVideoDescriptor.setupSizeLabel).toMatch(/26/);
    // No controls — auto-on-drop behavior.
    expect(reverseVideoDescriptor.controls).toBeUndefined();
  });
  ...
  it("rejects a non-MP4 file as UNSUPPORTED_INPUT", async () => {
    ...
    await expect(reverseVideoDescriptor.convert({ file })).rejects.toMatchObject({
  ...
  it("accepts an empty-MIME file with .mp4 extension past the MIME gate", async () => {
    ...
    await expect(
      reverseVideoDescriptor.convert({ file, signal: ctrl.signal }),
  ...
  it.skipIf(!ffmpegRuntimeAvailable)(
    "reverses a real MP4 (happy path)",
    async () => {
      await reverseVideoDescriptor.loadEngine!();
      ...
      const result = await reverseVideoDescriptor.convert({ file });
